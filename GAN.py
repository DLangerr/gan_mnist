...
class Discriminator:
    def __init__(self):
        with tf.variable_scope('d'):
            self.W_c1 = init_weights([5, 5, 1, 16])
            self.b_c1 = init_bias([16])
            self.W_c2 = init_weights([3, 3, 16, 32])
            self.b_c2 = init_bias([32])
    ...
    def forward(self, X):
        ...
        conv1 = tf.nn.leaky_relu(tf.nn.conv2d(self.X, self.W_c1, strides=[1, 2, 2, 1], padding='SAME') + self.b_c1)
        conv1 = tf.layers.batch_normalization(conv1, True)
        conv2 = tf.nn.leaky_relu(tf.nn.conv2d(conv1, self.W_c2, strides=[1, 2, 2, 1], padding='SAME') + self.b_c2)
